...
(defcustom sumibi-api-max-retries 1
  "同期 HTTP リクエストで応答が得られなかった場合の最大リトライ回数。
`url-retrieve-synchronously' が nil を返す (古い keep-alive 接続の
使い回しによる接続失敗など) ことがあるため、その場合に新しい接続で
再試行する回数を指定する。0 を指定するとリトライしない。"
  :type  'integer
  :group 'sumibi)
...
(defcustom sumibi-ambient-punctuation-delay 0.5
  "句読点トリガーの自動変換に挿入する遅延秒数（debounce）。
0 または nil の場合は遅延なしで即時変換する（従来の動作）。
非ゼロの場合、句読点（`.', `,', `?'）入力時にこの秒数だけ待ち、
その間に次のコマンドが実行されればタイマーをキャンセルして変換しない。
小数点を含む数値（例: `3.14'）や、句読点直後に続けて入力する
ケース（例: `hai,sorede'）での誤変換を防ぐ。
助詞+スペースのトリガーには影響しない。"
  :type 'number
  :group 'sumibi)
...
(defun sumibi--http-retrieve-synchronously (url)
  "URL に同期 HTTP リクエストを送り (status-code . body) を返す。

`url-retrieve-synchronously' は、古い keep-alive 接続を使い回した際の
接続失敗などで nil を返すことがある (Issue #150)。そのため以下の対策を行う:

1. `url-http-attempt-keepalives' を nil に束縛し、毎回新しい接続を張る。
2. それでも応答が得られなかった (buf=nil) 場合は、新しい接続で
   `sumibi-api-max-retries' 回まで再試行する。

接続失敗の現れ方は2通りある:
  - `url-retrieve-synchronously' が nil を返す (タイムアウトや、古い
    keep-alive 接続を使い回した際に応答が空になる場合)
  - `file-error' などのエラーを signal する (接続拒否や接続リセット)
本関数はその両方を再試行の対象とし、すべての試行で失敗した場合は
504 のエラー応答を返す。"
  (let ((url-http-attempt-keepalives nil)   ; 古い keep-alive 接続の使い回しを避ける
        (max-attempts (1+ (max 0 sumibi-api-max-retries)))
        (attempt 0)
        (status-and-body nil))
    (while (and (null status-and-body) (< attempt max-attempts))
      (setq attempt (1+ attempt))
      (condition-case err
          (let ((buf (url-retrieve-synchronously url t t sumibi-api-timeout)))
            (sumibi-debug-print (format "attempt %d/%d: %s\n"
                                        attempt max-attempts
                                        (if buf (buffer-name buf) "nil (応答なし)")))
            (if buf
                (setq status-and-body (sumibi-parse-http-body buf))
              (when (< attempt max-attempts)
                (sumibi-debug-print "  応答が得られませんでした。新しい接続で再試行します...\n"))))
        (error
         (sumibi-debug-print (format "attempt %d/%d: 接続エラー: %s\n"
                                     attempt max-attempts
                                     (error-message-string err)))
         (when (< attempt max-attempts)
           (sumibi-debug-print "  新しい接続で再試行します...\n")))))
    (or status-and-body
...
(defvar-local sumibi--ambient-punctuation-timer nil
  "句読点トリガーの遅延変換用タイマー（バッファローカル）。
保留中のタイマーがあれば `timer' オブジェクトを保持し、なければ nil。")

(defun sumibi--ambient-cancel-punctuation-timer ()
  "保留中の句読点遅延タイマーがあればキャンセルする。"
  (when (timerp sumibi--ambient-punctuation-timer)
    (cancel-timer sumibi--ambient-punctuation-timer))
...
タイマー満了まで `pre-command-hook' によるキャンセルが起きなかった
ことが保証されているため、その場で本体処理を呼び出す。"
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
      (setq sumibi--ambient-punctuation-timer nil)
      (sumibi--ambient-do-punctuation-trigger char))))
  ...
  (when (and sumibi-ambient-enable
    ...
    (let ((char (char-before)))
      (cond
       ...
       ;; 句読点入力時: 直前のローマ字を変換（必要なら遅延）
       ((memq char sumibi-auto-convert-punctuation)
        ;; 既存の保留タイマーがあればキャンセル（連打時の安全のため）
        (sumibi--ambient-cancel-punctuation-timer)
        (if (or (null sumibi-ambient-punctuation-delay)
                (<= sumibi-ambient-punctuation-delay 0))
            ;; 遅延なし: 従来動作で即時変換
            (sumibi--ambient-do-punctuation-trigger char)
          ;; 遅延あり: タイマーをセットして次コマンドまで待つ
          (sumibi-debug-print (format "sumibi-check-particle-trigger: scheduling punctuation conversion in %.2fs\n"
                                      sumibi-ambient-punctuation-delay))
          (setq sumibi--ambient-punctuation-timer
                (run-with-timer sumibi-ambient-punctuation-delay nil
                                #'sumibi--ambient-punctuation-timer-fire
                                char (current-buffer)))))))))

(defun sumibi--ambient-do-punctuation-trigger (char)
  "句読点 CHAR をトリガーとした自動変換の本体処理。
カーソル直前に CHAR が存在することを前提とし、ローマ字部分を日本語に
変換した上で、CHAR を全角句読点に置き換えて再挿入する。"
  (let* ((end (save-excursion
         ...
         (line-text (save-excursion
                      ...
                      (let ((current (point)))
                        (skip-chars-backward sumibi-skip-chars (line-beginning-position))
                        (buffer-substring-no-properties (point) current)))))
    (sumibi-debug-print (format "sumibi--ambient-do-punctuation-trigger: text='%s' start=%d end=%d\n" text start end))
    ;; 英文判定: 行全体が英文の場合は自動変換をスキップ（句読点も半角のまま）
    (if (sumibi-is-english-text-p line-text)
        (sumibi-debug-print (format "sumibi--ambient-do-punctuation-trigger: English text detected, skipping auto-convert: '%s'\n" line-text))
      ;; ローマ字比率判定: ローマ字変換可能な文字が閾値以上かチェック
      (if (not (sumibi-has-sufficient-romaji-p line-text))
          (sumibi-debug-print (format "sumibi--ambient-do-punctuation-trigger: Insufficient romaji ratio (%.2f < %.2f), skipping: '%s'\n"
                                      (sumibi-romaji-ratio line-text)
                                      sumibi-auto-convert-romaji-threshold
                                      line-text))
        ;; ローマ字テキストが存在する場合のみ変換
        (when (> (length text) 0)
          (sumibi-debug-print "sumibi--ambient-do-punctuation-trigger: punctuation detected! executing auto-convert\n")
          ;; 句読点を削除
          (delete-char -1)
          ;; 自動変換を実行
          (sumibi-rK-trans)
