...
- **1 秒未満でバランス良好**  \
  - `gemini-2.0-flash`：**0.74 s / 21.2 % / $0.00013** ─ 最速級で低コスト。  \

- **1 秒未満だがコストが高い**  \
  - `gpt-4o`：**0.98 s / 13.0 % / $0.0065** ─ 高精度だが高価。  \
  - `gemini-2.0-flash-lite`：0.66 s / 33.1 % / $0.0000975 ─ 最安だが精度は控えめ。  \
...
- **1 秒以上で実用性が落ちる**  \
  ...
  - `gemini-2.5-flash`：4.20 s / 11.0 % / $0.00065 ─ 高精度で中コスト。  \
  ...
  - `gemini-2.5-pro`：18.83 s / 4.8 % / $0.001625 ─ **最高精度**で低コストだが非常に遅い。  \
  - `gpt-5(medium)`：34.86 s / 4.6 % / $0.002625 ─ 高精度だが遅延が非常に大きい (reasoning_effort=medium)。  \
...
2. **精度重視**: `gemini-2.5-pro` が第一選択。**最高精度**（CER 4.8%）で相対的に低コスト。遅延18.8sを許容できるなら最高コスパ。

3. **中精度・中コスト**: `gemini-2.5-flash`（CER 11.0%、4.2s、$0.00065）は価格改定により従来ほどのコスパではないが、依然として実用的。
