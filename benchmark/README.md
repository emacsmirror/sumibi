...
- **実用性に課題（2秒超過でストレスフル）**  \
  ...
  - `gpt-5.5`：**4.23 s / 2.7 % / $0.0085** ─ **GPT-5系最高精度（CER 2.7%）**。ただし応答時間4.23sでIMEとしては実用性に課題。  \
...
- `gpt-5.5`（4.23s / **CER 2.7%** / $0.0085）: **GPT-5系最高精度**だが応答時間が実用限界を超える
- `gemini-2.5-pro`（18.83s）: 最高精度でも遅延が致命的
- `o3`（12.77s）: 高精度だが日常使用には不適

**結論**: IMEとしての実用性を考慮すると、**2秒以内の応答性**が必須条件。現実的な選択肢は1秒前後で動作するモデルに限られ、その中で `gemini-2.0-flash` が総合的に最も実用的。なお、`gemini-3.1-flash-lite-preview` は低コストながらCER 13.3%と高精度を実現しており、コストパフォーマンス重視の選択肢として有力。`gpt-5.5` はGPT-5系で最高精度（CER 2.7%）を達成しているが、4.23sの応答時間はIMEとして実用域を超えており、API高速化が待たれる。コストも$0.0085と高めで、精度と速度・コストのバランスでは現時点で `gpt-5.4` が実用上の最適解。
